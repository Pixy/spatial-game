...
import {
    GAME_REPOSITORY,
    PLAYER_REPOSITORY,
} from '../../infrastructure/modules/persistence.module';
...
export class GameUseCasesImpl implements GameUseCases {
  ...
  async placeItem(dto: PlaceItemDto): Promise<Game> {
    ...
    if (
      !this.gameDomainService.canPlayerMakeMove(game, dto.playerId, position)
    ) {
  ...
  async endGame(gameId: GameId): Promise<Game> {
    ...
    const totalHappiness = this.gameDomainService.calculateHappinessScore(
      game,
      animalPositions,
    );
